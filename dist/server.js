...
const server = (0, node_http_1.createServer)(async (request, response) => {
    try {
        ...
        if (request.method === "GET" && url.pathname === "/api/config") {
            sendJson(response, {
                allowedBetsCentavos: config_js_1.ALLOWED_BETS_CENTAVOS,
                allowedBetsPesos: config_js_1.ALLOWED_BETS_PESOS,
                symbolLabels
            });
            return;
        }
        if (request.method === "POST" && url.pathname === "/api/spin") {
            ...
            const result = (0, engine_js_1.spin)({ baseBet: parseBaseBet(body), mode: "base", source: "paid_spin" }, rng);
            sendJson(response, { roundType: "base_spin", result: serializeSpin(result) });
            return;
        }
        if (request.method === "POST" && url.pathname === "/api/bonus-buy") {
            const body = await readJsonBody(request);
            const result = (0, engine_js_1.buyBonus)({ baseBet: parseBaseBet(body) }, rng);
...
function serializeBonusBuy(result) {
...
function serializeBattleFeature(feature) {
    return {
